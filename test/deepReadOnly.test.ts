import {
  deepReadonly,
  type DeepReadonly,
} from "../utils/deepReadOnly/deepReadOnly";
import { expect } from "chai";

describe("deepReadonly", () => {
  it("should only accept objects", () => {
    // @ts-expect-error
    expect(deepReadonly.bind(null, "value")).to.throw;
    // @ts-expect-error
    expect(deepReadonly.bind(null, 1)).to.throw;
    // @ts-expect-error
    expect(deepReadonly.bind(null, Symbol("key"))).to.throw;
    // @ts-expect-error
    expect(deepReadonly.bind(null, function () {})).to.throw;
    // @ts-expect-error
    expect(deepReadonly.bind(null, true)).to.throw;
    // @ts-expect-error
    expect(deepReadonly.bind(null, null)).to.throw;
    // @ts-expect-error
    expect(deepReadonly.bind(null, undefined)).to.throw;
  });
